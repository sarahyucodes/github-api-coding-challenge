...
export const Search = () => {
  ...
  const renderResults = () => {
    if (status === 'loading') {
      ...
      return <p className='error-message p-2 text-sm'>{error}</p>
        ...
        <ul className='py-1'>
          {
            results.map(result => {
              return (
                ...
                >
                  <span className='col-span-6 mr-4'>{result.fullName}</span>
                  ...
                  <span className='col-span-1 text-xs flex items-center'>
                    ...
                    <span className='ml-1'>{result.stargazersCount}</span>
                  </span>
                </li>
              )
            })
